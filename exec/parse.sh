...
then
	...
	g++ $SCRATCH/droplet/src/substratePosition2D.cpp -o $SCRATCH/droplet/exec/substratePosition2D.out
	g++ $SCRATCH/droplet/src/substrateDensity.cpp -o $SCRATCH/droplet/exec/substrateDensity.out
...
#	#Extract substrate atoms from large file
	...
	$SCRATCH/droplet/exec/substrateDensity.out $out_folder/substrate.txt $out_folder/substrate_density_halfA.txt $out_folder/center_of_mass.txt
	rm $out_folder/substrate.txt

	echo "Calculating substrate position"
	$SCRATCH/droplet/exec/substratePosition2D.out $out_folder/substrate.txt $out_folder/substrate_position.txt
