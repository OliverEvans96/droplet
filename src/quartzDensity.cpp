...
//Masses of atoms
const double masses[5] = {28.0899,15.9999,28.0899,15.9999,1.0080};
...
void readStep(ifstream &inFile,double *coords,int *elements,int numAtoms,int numSteps,int &timestep,string *arr);

//Calculate density(z) for one timestep
void calcDensity(int numAtoms,double *coords,int *elements,int nZ,double zWidth,double *zVals,double *density,double *center);
	...
	else
	{
		...
		try
		{
			...
			for(int step=0;step<numSteps;step++)
			{
				...
				//Read data
				readStep(inFile,coords,elements,numAtoms,numSteps,timestep,arr);

				//Calculate center of mass
				centerOfMass(numAtoms,coords,elements,center);

				//Density Calculations
				calcDensity(numAtoms,coords,elements,nZ,zWidth,zVals,density,center);
...
void calcDensity(int numAtoms,double *coords,int *elements,int nZ,double zWidth,double *zVals,double *density,double *center)
{
	//x-y area is 171312.33624 AA^2
	double volume=171312.33624*zWidth;
