//Calculate position of z-center of mass as a function of x,y for sapphire
...
void centerOfMass(int numAtoms,double *coords,int *elements,double **zbar,int nx,int ny,double *bounds,double binLength,double totalMass);

//Write data for one timestep
void writeStep(ofstream &centerFile,double **zbar,int timestep,int nx,int ny);
	...
	else
	{
		...
		try
		{
			...
			for(int step=0;step<numSteps;step++)
			{
				...
				centerOfMass(numAtoms,coords,elements,zbar,nx,ny,bounds,binLength,totalMass);

				// Write data
				writeStep(centerFile,zbar,timestep,nx,ny);
...
void centerOfMass(int numAtoms,double *coords,int *elements,double **zbar,int nx,int ny,double *bounds,double binLength,double totalMass)
{
	...
	for(int k=0;k<numAtoms;k++)
	{
		...
		zbar[i][j] += (z * masses[*elements++ - 1]) / totalMass;
	}
...
{
	//Write timestep
	centerFile << "# TIMESTEP " << timestep << endl;

	//Write center of mass
	for(int i=0;i<nx;i++)
	{
		for(int j=0;j<ny;j++)
			centerFile << setprecision(5) << setw(8) << zbar[i][j] << " ";
		centerFile << endl;
	}
